...
import ViewServiceModal from '@/components/modals/service/ViewServiceModal.vue'
  ...
  setup() {
    ...
    const handleServiceCreated = async (serviceData) => {
      try {
        ...
        // Close modal
        if (addServiceModalRef.value && addServiceModalRef.value.modalRef) {
          const modalEl = addServiceModalRef.value.modalRef
          ...
          const modalEl = document.getElementById('add_service')
          const Bootstrap = window.bootstrap ?? window.Bootstrap
    ...
    const handleServiceUpdated = async (serviceData) => {
      try {
        ...
        // Close modal
        if (editServiceModalRef.value && editServiceModalRef.value.modalRef) {
          const modalEl = editServiceModalRef.value.modalRef
          const Bootstrap = window.bootstrap ?? window.Bootstrap
          if (Bootstrap) {
            const modal = Bootstrap.Modal.getInstance(modalEl)
            if (modal) modal.hide()
          ...
          // Fallback
          const modalEl = document.getElementById('edit_service')
          const Bootstrap = window.bootstrap ?? window.Bootstrap
          if (Bootstrap && modalEl) {
            const modal = Bootstrap.Modal.getInstance(modalEl)
            if (modal) modal.hide()
