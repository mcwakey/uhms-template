    ...
    <div class="content" id="profilePage">
      ...
      >
        <div class="flex-grow-1">
          <h4 class="fw-bold mb-0">
            Assets<span class="badge badge-soft-primary border border-primary fs-13 fw-medium ms-2"
              >Total: {{ AssetsTable.totalCount }}</span
            >
          </h4>
        </div>
        <div class="text-end d-flex">
          ...
          <RouterLink to="/hrm/staff/add" class="btn btn-primary ms-2 fs-13 btn-md">
            <i class="ti ti-plus me-1"></i>Add New Asset
          </RouterLink>
        </div>
      </div>
      <!-- End Page Header -->
      ...
      <div class="table-responsive">
        <a-table
          ...
          :columns="columns"
          :table-layout="fixed"
          :data-source="AssetsTable.data.value"
          :pagination="paginationConfig"
          @change="AssetsTable.handleTableChange"
          row-key="id"
...
export default {
  ...
  setup() {
    const AssetsTable = useTableStore('assets')
    const detailedItem = computed(() => AssetsTable.detailedItem.value || {})

    const paginationConfig = computed(() => ({
      current: AssetsTable.currentPage.value,
      pageSize: AssetsTable.perPage.value,
      total: AssetsTable.totalCount.value,
    ...
    const openModal = async (record) => {
      try {
        AssetsTable.selectItem(record)
        await AssetsTable.fetchItemDetails(record.uuid)
      } catch (error) {
        message.error(error)
      }
    }
