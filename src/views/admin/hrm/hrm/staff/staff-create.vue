...
const onSubmit = async () => {
  ...
  //   return;
  // }
  // notifyError('Staff created successfully');
}
...
const convertDate = () => {
  ...
  // Debug: Log the education and experience entries before conversion
  console.log('Education entries before conversion:', educationEntries.value)
  console.log('Experience entries before conversion:', experienceEntries.value)

  // Safely convert date fields using helper function
  ...
  // If only one contact, send as object; if more, send as array of objects
  if (emergencyContacts.value.length > 1) {
  ...
  // Include education and experience from repeaters
  payload.education_experience = [
    ...educationEntries.value.map((entry) => {
      const convertedEntry = {
        ...entry,
        year_from: formatDateToYYYYMMDD(entry.year_from),
        year_to: formatDateToYYYYMMDD(entry.year_to),
        type: 'education',
      }
      console.log('Converting education entry:', entry, '→', convertedEntry)
      return convertedEntry
    }),
    ...experienceEntries.value.map((entry) => {
      const convertedEntry = {
        ...entry,
        year_from: formatDateToYYYYMMDD(entry.year_from),
        year_to: formatDateToYYYYMMDD(entry.year_to),
      ...
      console.log('Converting experience entry:', entry, '→', convertedEntry)
      return convertedEntry
    }),
  ]

  console.log('Final education_experience payload:', payload.education_experience)
