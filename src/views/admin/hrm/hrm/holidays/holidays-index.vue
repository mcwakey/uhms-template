    ...
    <div class="content" id="profilePage">
      ...
      >
        <div class="flex-grow-1">
          <h4 class="fw-bold mb-0">
            Holidays<span
              ...
              >Total: {{ HolidaysTable.totalCount }}</span
            >
          </h4>
        </div>
        <div class="text-end d-flex">
          ...
          <RouterLink to="/hrm/staff/add" class="btn btn-primary ms-2 fs-13 btn-md">
            <i class="ti ti-plus me-1"></i>Add New Holiday
          </RouterLink>
        </div>
      </div>
      <!-- End Page Header -->
      ...
      <div class="table-responsive">
        <a-table
          ...
          :columns="columns"
          :table-layout="fixed"
          :data-source="HolidaysTable.data.value"
          :pagination="paginationConfig"
          @change="HolidaysTable.handleTableChange"
          row-key="id"
...
export default {
  ...
  setup() {
    const HolidaysTable = useTableStore('holidays')
    const detailedItem = computed(() => HolidaysTable.detailedItem.value || {})

    const paginationConfig = computed(() => ({
      current: HolidaysTable.currentPage.value,
      pageSize: HolidaysTable.perPage.value,
      total: HolidaysTable.totalCount.value,
    ...
    const openModal = async (record) => {
      try {
        HolidaysTable.selectItem(record)
        await HolidaysTable.fetchItemDetails(record.uuid)
      } catch (error) {
        message.error(error)
      }
    }
