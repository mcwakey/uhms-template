    ...
    <div class="content" id="profilePage">
      ...
      >
        <div class="flex-grow-1">
          <h4 class="fw-bold mb-0">
            Payrolls<span
              ...
              >Total: {{ PayrollsTable.totalCount }}</span
            >
          </h4>
        </div>
        <div class="text-end d-flex">
          ...
          <RouterLink to="/hrm/staff/add" class="btn btn-primary ms-2 fs-13 btn-md">
            <i class="ti ti-plus me-1"></i>Add New Payroll
          </RouterLink>
        </div>
      </div>
      <!-- End Page Header -->
      ...
      <div class="table-responsive">
        <a-table
          ...
          :columns="columns"
          :table-layout="fixed"
          :data-source="PayrollsTable.data.value"
          :pagination="paginationConfig"
          @change="PayrollsTable.handleTableChange"
          row-key="id"
...
export default {
  ...
  setup() {
    const PayrollsTable = useTableStore('payrolls')
    const detailedItem = computed(() => PayrollsTable.detailedItem.value || {})

    const paginationConfig = computed(() => ({
      current: PayrollsTable.currentPage.value,
      pageSize: PayrollsTable.perPage.value,
      total: PayrollsTable.totalCount.value,
    ...
    const openModal = async (record) => {
      try {
        PayrollsTable.selectItem(record)
        await PayrollsTable.fetchItemDetails(record.uuid)
      } catch (error) {
        message.error(error)
      }
    }
