    ...
    <div class="content" id="profilePage">
        ...
        <div class="flex-grow-1">
          <h4 class="fw-bold mb-0">
            Designations<span
              ...
              >Total: {{ DesignationsTable.totalCount }}</span
            >
          </h4>
        </div>
      ...
      <div class="table-responsive">
        <a-table
          ...
          :columns="columns"
          :table-layout="fixed"
          :data-source="DesignationsTable.data.value"
          :pagination="paginationConfig"
          @change="DesignationsTable.handleTableChange"
          row-key="id"
...
export default {
  ...
  setup() {
    const DesignationsTable = useTableStore('designations')
    const detailedItem = computed(() => DesignationsTable.detailedItem.value || {})

    const paginationConfig = computed(() => ({
      current: DesignationsTable.currentPage.value,
      pageSize: DesignationsTable.perPage.value,
      total: DesignationsTable.totalCount.value,
    ...
    const openModal = async (record) => {
      try {
        DesignationsTable.selectItem(record)
        await DesignationsTable.fetchItemDetails(record.uuid)
      } catch (error) {
        message.error(error)
      }
    }
    ...
    onMounted(() => {
      ...
      DesignationsTable.fetchData().catch(() => {
