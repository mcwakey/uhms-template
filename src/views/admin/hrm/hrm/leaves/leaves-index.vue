    ...
    <div class="content" id="profilePage">
      ...
      >
        <div class="flex-grow-1">
          <h4 class="fw-bold mb-0">
            Leaves<span class="badge badge-soft-primary border border-primary fs-13 fw-medium ms-2"
              >Total: {{ LeavesTable.totalCount }}</span
            >
          </h4>
        </div>
        <div class="text-end d-flex">
          ...
          <RouterLink to="/hrm/staff/add" class="btn btn-primary ms-2 fs-13 btn-md">
            <i class="ti ti-plus me-1"></i>Add New Leave
          </RouterLink>
        </div>
      </div>
      <!-- End Page Header -->
      ...
      <div class="table-responsive">
        <a-table
          ...
          :columns="columns"
          :table-layout="fixed"
          :data-source="LeavesTable.data.value"
          :pagination="paginationConfig"
          @change="LeavesTable.handleTableChange"
          row-key="id"
...
export default {
  ...
  setup() {
    const LeavesTable = useTableStore('leaves')
    const detailedItem = computed(() => LeavesTable.detailedItem.value || {})

    const paginationConfig = computed(() => ({
      current: LeavesTable.currentPage.value,
      pageSize: LeavesTable.perPage.value,
      total: LeavesTable.totalCount.value,
    ...
    const openModal = async (record) => {
      try {
        LeavesTable.selectItem(record)
        await LeavesTable.fetchItemDetails(record.uuid)
      } catch (error) {
        message.error(error)
      }
    }
