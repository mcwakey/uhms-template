    ...
    <div class="content" id="profilePage">
      ...
      >
        <div class="flex-grow-1">
          <h4 class="fw-bold mb-0">
            Payments<span
              ...
              >Total: {{ PaymentsTable.totalCount }}</span
            >
          </h4>
        </div>
        <div class="text-end d-flex">
          ...
          <RouterLink to="/hrm/staff/add" class="btn btn-primary ms-2 fs-13 btn-md">
            <i class="ti ti-plus me-1"></i>Add New Payment
          </RouterLink>
        </div>
      </div>
      <!-- End Page Header -->
      ...
      <div class="table-responsive">
        <a-table
          ...
          :columns="columns"
          :table-layout="fixed"
          :data-source="PaymentsTable.data.value"
          :pagination="paginationConfig"
          @change="PaymentsTable.handleTableChange"
          row-key="id"
...
export default {
  ...
  setup() {
    const PaymentsTable = useTableStore('payments')
    const detailedItem = computed(() => PaymentsTable.detailedItem.value || {})

    const paginationConfig = computed(() => ({
      current: PaymentsTable.currentPage.value,
      pageSize: PaymentsTable.perPage.value,
      total: PaymentsTable.totalCount.value,
    ...
    const openModal = async (record) => {
      try {
        PaymentsTable.selectItem(record)
        await PaymentsTable.fetchItemDetails(record.uuid)
      } catch (error) {
        message.error(error)
      }
    }
