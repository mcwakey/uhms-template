    ...
    <div class="content" id="profilePage">
        ...
        <div class="flex-grow-1">
          <h4 class="fw-bold mb-0">
            Expenses<span
              ...
              >Total: {{ ExpensesTable.totalCount }}</span
            >
          </h4>
        </div>
      ...
      <div class="table-responsive">
        <a-table
          ...
          :columns="columns"
          :table-layout="fixed"
          :data-source="ExpensesTable.data.value"
          :pagination="paginationConfig"
          @change="ExpensesTable.handleTableChange"
          row-key="id"
...
export default {
  ...
  setup() {
    const ExpensesTable = useTableStore('expenses')
    const detailedItem = computed(() => ExpensesTable.detailedItem.value || {})

    const paginationConfig = computed(() => ({
      current: ExpensesTable.currentPage.value,
      pageSize: ExpensesTable.perPage.value,
      total: ExpensesTable.totalCount.value,
    ...
    const openModal = async (record) => {
      try {
        ExpensesTable.selectItem(record)
        await ExpensesTable.fetchItemDetails(record.uuid)
      } catch (error) {
        message.error(error)
      }
    }
