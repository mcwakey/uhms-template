    ...
    <div class="content" id="profilePage">
      ...
      >
        <div class="flex-grow-1">
          <h4 class="fw-bold mb-0">
            Incomes<span class="badge badge-soft-primary border border-primary fs-13 fw-medium ms-2"
              >Total: {{ IncomesTable.totalCount }}</span
            >
          </h4>
        </div>
      ...
      <div class="table-responsive">
        <a-table
          ...
          :columns="columns"
          :table-layout="fixed"
          :data-source="IncomesTable.data.value"
          :pagination="paginationConfig"
          @change="IncomesTable.handleTableChange"
          row-key="id"
...
export default {
  ...
  setup() {
    const IncomesTable = useTableStore('incomes')
    const detailedItem = computed(() => IncomesTable.detailedItem.value || {})

    const paginationConfig = computed(() => ({
      current: IncomesTable.currentPage.value,
      pageSize: IncomesTable.perPage.value,
      total: IncomesTable.totalCount.value,
    ...
    const openModal = async (record) => {
      try {
        IncomesTable.selectItem(record)
        await IncomesTable.fetchItemDetails(record.uuid)
      } catch (error) {
        message.error(error)
      }
    }
